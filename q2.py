        ...
        for i in range(d+1):
            if i == 0:
                ...
                if len(temp_graph) <= n_pop:
                    ...
                    hitler_set = set(random.sample(
                        list(temp_graph.keys()), n_pop))
...
def plot_strenth_curve(city_name, l, output_dir="performance_data", compare=False, adjusted=False):
    ...
    if compare and not adjusted:
        ...
        img_path = output_path / \
            f"{city_name}_compare_strenth_curve_adjusted.png"
    elif not compare and not adjusted:
...
def main():
    ...
    for city_file in city_files:
        ...
        # 调用清洗函数
        G, _ = get_processed_graph(
            df, city_name=city_name, plot=False, include_length=False)
...
def related():
    ...
    for city_file in city_files:
        ...
        plot_strenth_curve(city_name, grid_line,
                           output_dir="performance_data", compare=True)

        grid_line_ad = adjust(grid_line)

        plot_strenth_curve(city_name, grid_line_ad,
                           output_dir="performance_data", compare=True, adjusted=True)
...
def stablity(G, nlist, blist, rep):
    ...
    for n in nlist:
        ...
        for b in blist:
            ...
            print(
                f"n={n}, b={b}: mean={mean_val:.6f}, std={std_val:.6f}, rstd={rstd:.6f}")
    ...
    for i, n in enumerate(nlist):
        row_str = f"{n:<8}" + \
            "".join([f"{rstd_matrix[i][j]:>12.6f}" for j in range(len(blist))])
        print(row_str)
...
# 绘制稳健性曲线,dn为计算稳健性的参数,DN为绘制曲线的参数
def plot_robustness_line(G, d, n, D, N, city_name="City", output_dir="robustness_line"):
...
def plot_eccentricity_distribution(csv_path, city_name="", output_dir="eccentricity"):
    ...
    G, node_component = get_processed_graph(
        df, city_name=city_name, plot=False, include_length=True)
    print(f"原始节点数: {len(G)}")
    ...
    np.savetxt(data_path, ecc_sorted, delimiter=',',
               header='eccentricity', comments='')
...
def read_threshold(city_name, target1=0.8, target2=0.2):
    ...
    script_dir = Path(__file__).parent
    # 读取调整后的数据（已标准化）
    csv_path = script_dir / "performance_data" / \
        f"{city_name}_strenth_data_adjusted.csv"
    if not csv_path.exists():
        # 如果调整后的文件不存在，尝试读取未调整的
        csv_path = script_dir / "performance_data" / \
            f"{city_name}_strenth_data.csv"
        if not csv_path.exists():

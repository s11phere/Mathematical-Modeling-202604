    ...
    for t in range(N):
        ...
        for i in range(d+1):
            if i == 0:
                ...
                n_pop = x[i]-x[i-1]
                if len(temp_graph) <= n_pop:
                    hitler_set = set(temp_graph.keys())
                else:
...
def plot_strenth_curve(city_name, l, output_dir="performance_data",compare = False,adjusted = False):
    ...
    if adjusted:
        csv_path = output_path / f"{city_name}_strenth_data_adjusted.csv"
    else:
...
def read_threshold(city_name, target1=0.8, target2=0.2):
    """
    从 performance_data 文件夹中读取指定城市的效能曲线数据，
    计算标准化曲线 y/(1-x) = target1 和 target2 对应的移除比例。
    注意：这里读取的是调整后的数据（文件名为 *_strenth_data_adjusted.csv），
          因为调整后的数据已经是 y/(1-x)，无需再次标准化。
    """
    script_dir = Path(__file__).parent
    # 读取调整后的数据（已标准化）
    csv_path = script_dir / "performance_data" / f"{city_name}_strenth_data_adjusted.csv"
    if not csv_path.exists():
        # 如果调整后的文件不存在，尝试读取未调整的
        csv_path = script_dir / "performance_data" / f"{city_name}_strenth_data.csv"
        if not csv_path.exists():
            raise FileNotFoundError(f"未找到文件: {csv_path}")
        # 如果是未调整的，需要计算标准化
        df = pd.read_csv(csv_path)
        x = df['Fraction_Removed'].values
        y = df['Normalized_Size'].values
        mask = x < 1.0
        x_clean = x[mask]
        y_clean = y[mask]
        y_prime = y_clean / (1.0 - x_clean)
        print(f"读取未调整数据，计算标准化 y/(1-x)")
    else:
        df = pd.read_csv(csv_path)
        x = df['Fraction_Removed'].values
        y = df['Normalized_Size'].values
        mask = x < 1.0
        x_clean = x[mask]
        y_prime = y[mask]   # 调整后的文件中的 y 已经是标准化值
        print(f"读取调整后的数据，直接使用 y 作为标准化值")

    def find_x(target):
        idx = np.where(y_prime <= target)[0]
        if len(idx) == 0:
            return 1.0
        i = idx[0]
        if i == 0:
            return x_clean[0]
        x1, x2 = x_clean[i-1], x_clean[i]
        y1, y2 = y_prime[i-1], y_prime[i]
        if y2 == y1:
            return x1
        x_target = x1 + (target - y1) * (x2 - x1) / (y2 - y1)
        return max(0.0, min(1.0, x_target))

    x_0_8 = find_x(target1)
    x_0_2 = find_x(target2)
    diff = x_0_2 - x_0_8
    mean = (x_0_2 + x_0_8) / 2.0
    return x_0_8, x_0_2, diff, mean
...
def site_percolation_threshold(graph_input, tol=1e-6, max_iter=1000, return_pc=False):
    """
    基于消息传递算法计算给定图随机点渗流的临界保留概率 p_c。
    该算法精确考虑了网络中的任意环路，适用于城市道路网等含环图。

    参数
    ----------
    graph_input : dict 或 str
        - dict: 邻接表格式 {node: [neighbor1, neighbor2, ...]}
        - str: CSV 文件路径，必须包含 'START_NODE', 'END_NODE' 列
    tol : float, default=1e-6
        消息迭代收敛容差
    max_iter : int, default=1000
        最大迭代次数
    return_pc : bool, default=False
        若为 True，则同时返回 (p_c, q_c)；否则只返回 q_c

    返回
    -------
    q_c : float
        理论临界移除比例 (0~1)，即 1 - p_c
    p_c : float (可选)
        理论临界保留概率
    """
    # ------------------- 1. 读取图 -------------------
    if isinstance(graph_input, str):
        df = pd.read_csv(graph_input)
        required = ['START_NODE', 'END_NODE']
        if not all(col in df.columns for col in required):
            raise ValueError("CSV 缺少 START_NODE / END_NODE 列")
        G = nx.Graph()
        for _, row in df.iterrows():
        ...
        raise TypeError("输入必须是字典或 CSV 文件路径")
    ...
    # 重新标记节点为 0..N-1，便于数组索引
    node_list = list(G.nodes())
    node_to_idx = {node: i for i, node in enumerate(node_list)}
    N = len(node_list)
    adj = {i: [node_to_idx[v] for v in G.neighbors(node_list[i])] for i in range(N)}
    deg = [len(adj[i]) for i in range(N)]

    # ------------------- 2. 消息传递求解临界保留概率 -------------------
    # 对于每条有向边 i->j，定义消息 h_{i->j} ∈ [0,1]
    # 初始化所有消息为 0.5
    messages = {}
    for i in range(N):
        for j in adj[i]:
    ...
    # 外循环：对保留概率 p 进行二分搜索，找到使巨分量出现的最小 p
    # 消息传递固定点方程：h_{i->j} = 1 - p * ∏_{k∈N(i)\j} (1 - h_{k->i})
    # 对于给定的 p，迭代至收敛，然后计算每个节点的“空腔场”，进而得到巨分量占比。
    # 临界 p_c 定义为巨分量占比 > 0 的最小 p。
    
    # 更简单的方法：直接利用消息传递求 p_c 的公式（不需要二分）：
    # 定义向量 h，然后求解特征值问题？但一般迭代即可。
    # 实际算法：对于给定的 p，定义算子 F_p，其不动点可判断是否存在巨分量。
    # 临界 p_c 满足最大特征值 = 1。可以通过对 p 二分搜索实现。
    
    def giant_component_fraction(p):
        """给定保留概率 p，返回估计的巨分量相对大小（通过消息传递）"""
        # 初始化消息
        h = {edge: 0.5 for edge in messages}
        for _ in range(max_iter):
            ...
            for (i, j), val in h.items():
                ...
                for k in adj[i]:
                    if k == j:
                        continue
                    prod *= (1.0 - h.get((k, i), 0.0))
                new_val = 1.0 - p * prod
                new_h[(i, j)] = new_val
                max_diff = max(max_diff, abs(new_val - val))
            h = new_h
            if max_diff < tol:
                break
        # 计算每个节点的存在概率
        ...
        for i in range(N):
            prod = 1.0
            for j in adj[i]:
                prod *= (1.0 - h.get((j, i), 0.0))
            node_prob.append(1.0 - p * prod)
        # 巨分量相对大小 ≈ 节点存在概率的平均（实际需要更精细处理，但作为判据足够）
        return np.mean(node_prob)

    # 二分搜索 p_c
    p_low, p_high = 0.0, 1.0
    for _ in range(60):  # 二分 60 次足够
        p_mid = (p_low + p_high) / 2
        size = giant_component_fraction(p_mid)
        if size > 1e-6:
            p_high = p_mid
        else:
            p_low = p_mid
    p_c = p_high
    q_c = 1.0 - p_c
    return (p_c, q_c) if return_pc else q_c
...
if __name__ == "__main__":
    ...
    for city_file in city_files:
        ...
        q_c = site_percolation_threshold(G)
        print(f"理论临界移除比例 q_c = {q_c:.4f}")